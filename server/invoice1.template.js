...
function makePages(items) {
    while (flag) {
        ...
        for (i = 0; i < items.length; i++) {
            const item = items[i];
            const noOfLines = Math.ceil((item.description.length / 41).toFixed(1));
            height += (noOfLines * 14);
            if (items.length > i + 1) {
                const nextItem = items[i + 1];
                const nextItemHeight = (Math.ceil((nextItem.description.length / 41).toFixed(1)) * 14);
...
module.exports.getContent = function (data) {
    ...
    * {
    ...
    .items-table .items-row>div:not(.no-padding) {
        padding: 0px 3px;
    }
...
function getCenter(items) {
    ...
    <div class="items-row font-bold">
        ...
        <div class="w-sgst text-center no-padding">
            <div class="separator row justify-content-center">
                <div>SGST</div>
            </div>
            <div class="separator row">
...
function getRows(items) {
    ...
    items.forEach((item, i) => {
        ...
        <div class="items-row">
            ...
            <div class="text-right w-taxable">${formatNumber((taxableAmount))}</div>
            <div class="text-right w-gst-rate">${item.sgst}</div>
            <div class="text-right w-gst-amt">${formatNumber((SGSTAmount))}</div>
            <div class="text-right w-gst-rate">${item.cgst}</div>
            <div class="text-right w-gst-amt">${formatNumber((CGSTAmount))}</div>
            <div class="text-right w-gst-rate">${item.igst}</div>
            <div class="text-right w-gst-amt">${formatNumber((IGSTAmount))}</div>
...
function getBottom(data, currentPage, totalPage, withEnd) {
    ...
    <div class="summary-info row">
        <div class="d-flex flex-column" ${hideCell(withEnd)}>
            ...
            <div class="separator row bb-0 stretch-height">
                ...
                <div class="padding text-right w-taxable-amt">${formatNumber((totalTaxable))}</div>
        ...
        <div class="w-100">
            <div class="padding">
                <b ${hideCell(withEnd)}>Total Amount In Words</b><br>
                <div class="text-capitalize" ${hideCell(withEnd)}>
                ${inrInWords.convert(roundPrecision(totalTaxable + totalSGST + totalCGST + totalIGST, 2))}
                </div>
            </div>
        </div>
        <div class="w-final-total" ${hideCell(withEnd)}>
    ...
    <div class="separator row">
        <div class="w-100 padding d-flex align-items-center">
            <strong>Note :&nbsp;&nbsp;</strong>
            <span>${(data.note || '').substr(0, 130)}</span>
        </div>
    </div>
